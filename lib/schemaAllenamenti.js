...
// allenamenti-2026-27), non in una tabella relazionale: per derivare lo stato di una
// Session Schema dal calendario reale (invece di un flag "eseguita" separato che
// potrebbe disallinearsi) leggiamo quel blob e ne ricaviamo gli id con data <= oggi.
export async function getPastAllenamentoIds(userId) {
  const entry = await prisma.kvEntry.findUnique({
    where: { userId_key: { userId, key: 'allenamenti-2026-27' } },
  });
